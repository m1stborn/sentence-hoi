...
def main(args):
    ...
    # print(img_tensor.size(), anno_dict.keys())
    for k, v in anno_dict.items():
        if torch.is_tensor(v):
            print(k, v)
    # print(anno_dict['human_labels'], anno_dict['image_id'])

    """
    dataset structure:
        (img_tensor, annot_dict)
    :param img_tensor:
        torch.Tensor, size: torch.Size([3, 512, 560])
    :param anno_dict:
        dict: {
    ...
    output = model(torch.rand((3, 3, 512, 512)).to(device))
