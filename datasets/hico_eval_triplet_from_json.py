...
class HICOEvaluatorJson:
    def __init__(self, result_json: str, rare_triplets, non_rare_triplets, args):
        ...
        try:
            ...
            """
            Result json is str format, need to convert to json format.
            """
            with open(result_json.replace(".json", "-mod.json"), "w") as file:
        ...
        for i, img_gts in enumerate(self.gts):
            for hoi in self.gts[i]['hoi_annotation']:
    ...
    def compute_map(self):
        ...
        for triplet in self.gt_triplets:
            ...
            # print("hico_eval_triplet_from_json", triplet)
            if len(tp) == 0:
                ap[triplet] = 0
        ...
        # TODO: save to csv
        # with open("result/result-ap.json", "w", encoding="utf-8") as file:
        #     json.dump(ap, file, ensure_ascii=False, indent=4)
        #
        # with open("result/result-rare-ap.json", "w", encoding="utf-8") as file:
        #     json.dump(rare_ap, file, ensure_ascii=False, indent=4)
        #
        # with open("result/result-nonrare-ap.json", "w", encoding="utf-8") as file:
        #     json.dump(non_rare_ap, file, ensure_ascii=False, indent=4)
