...
class HoiDetection(VisionDataset):
    ...
    """
    def __init__(self, root, ann_file, target_transform=None, transforms=None, image_set='train'):
        assert image_set in ['train', 'test'], image_set
        self.image_set = image_set
        super(HoiDetection, self).__init__(root, transforms, target_transform)
        annotations = [parse_one_gt_line(l.strip()) for l in open(ann_file, 'r').readlines()]
    ...
    def __getitem__(self, index):
        ...
        if 'train2015' in img_name:
            ...
            img_path = f"../gen-vlkt/data/hico_20160224_det/images/train2015/{img_name}"
        elif 'test2015' in img_name:
            # img_path = './data/hico/images/test2015/%s' % img_name
            img_path = f"../gen-vlkt/data/hico_20160224_det/images/test2015/{img_name}"
...
def build(image_set, test_scale=-1):
    ...
    dataset = HoiDetection(root='./data/hico', ann_file=annotation_file,
